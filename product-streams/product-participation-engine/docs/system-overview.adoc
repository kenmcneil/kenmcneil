...
Participation records are scheduled by the <<#participation-author-term,author>> to start and stop at designated times. Records are "activated" on the start date, and "deactivated" on the end date or when manually un-published by an author.
...
|Set the sale id of the entered products to `saleId`.

|Calculated Discounts
|`uniqueIds` list, `type`, `pb 1 discount`, `pb 22 discount`
|Apply flat or percentage discounts to the pricebook 1 and 22 prices of the entered products.
