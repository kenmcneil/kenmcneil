...
/**
 * This class is responsible for all database changes related to participation@2 records: saleId and calculated discounts.
 * It provides getters to retrieve values based of the path defined in the definition file
 * e.g
 * "content": {
 *     "_type": "participation@2.0.0",
 *     "productSale": {
 *       "saleId": 7574,
...
public class ParticipationV2Lifecycle implements ParticipationLifecycle {

	private static final Logger LOG = LoggerFactory.getLogger(ParticipationV2Lifecycle.class);
	...
	private final ParticipationV2Dao participationV2Dao;

	@Override
	public ParticipationContentType getContentType() {
		return ParticipationContentType.PARTICIPATION_V2;
	}
	...
	public int publish(ParticipationItem item, Date processingDate) {
		...
		rowsAffected += participationV2Dao.upsertParticipationCalculatedDiscounts(
	...
	public int activateEffects(ParticipationItemPartial itemPartial, Date processingDate) {
		...
		int rowsAffected = participationV2Dao.applyNewCalculatedDiscounts(processingDate, userId,
	...
	public int deactivateEffects(ParticipationItemPartial itemPartial, Date processingDate) {
		...
		int rowsAffected = participationV2Dao.updateLastOnSaleBasePrices(processingDate);
		totalRows += rowsAffected;
		LOG.debug("{}: {} lastOnSale basePrice values saved", participationId, rowsAffected);

		rowsAffected = participationV2Dao.takePricesOffSaleAndApplyPendingBasePriceUpdates(userId);
		totalRows += rowsAffected;
	...
	public int unpublish(ParticipationItemPartial itemPartial, Date processingDate) {
		...
		return participationCoreDao.deleteParticipationProducts(participationId)
				+ participationV2Dao.deleteParticipationCalculatedDiscounts(participationId)
	...
	public void publishToHistory(ParticipationItem item, Date processingDate) {
		...
		participationV2Dao.insertParticipationCalculatedDiscountsHistory(
