...
public class DynamicYieldReadListener implements StepExecutionListener {
    ...
    public ExitStatus afterStep(StepExecution stepExecution) {
        if (stepExecution.getReadCount() < dyFeedSettings.getMinimumRecordCount()) {
            LOGGER.error("Row Count < " + dyFeedSettings.getMinimumRecordCount() + " rows!");
            return new ExitStatus(ExitStatus.FAILED.getExitCode(), "Row Count < " +
                    dyFeedSettings.getMinimumRecordCount() + " rows!");
        }
